...
def get_network_by_industry(firstlastp):
    cand = ("'"+ str(firstlastp) +"'")
    row = query_pg("SELECT * FROM network_industry t1 LEFT JOIN (select distinct(Industry) FROM network_industry WHERE firstlastp = {cand})sub ON t1.Industry = sub.Industry WHERE (sub.Industry IS NOT NULL) and (t1.contr_amt> 1)", (cand))
